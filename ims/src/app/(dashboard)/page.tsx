...
import { SysAdminDashboard } from "@/components/dashboard/sysadmin-dashboard";
import { KabemDashboard } from "@/components/dashboard/kabem-dashboard";
...
// All static fallback constants deleted to ensure no dummy data
...
export default function DashboardPage() {
  ...
  const isGuest = activeRole === "Guest" || !activeRole;
    ...
    return (
      <div className="flex min-h-[70vh] items-center justify-center p-4">
        <motion.div
          ...
          className="relative z-10 w-full max-w-lg overflow-hidden rounded-[32px] border border-white/10 bg-white/5 p-8 md:p-10 shadow-[0_32px_100px_rgba(0,0,0,0.4)] backdrop-blur-2xl text-center"
        >
          {/* Ambient Glow Orb */}
          <div className="pointer-events-none absolute -right-20 -top-20 h-48 w-48 rounded-full bg-[#10b981]/20 blur-[60px]" />

          <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl border border-[#f0c36a]/30 bg-[#f0c36a]/10 mb-6">
            <AlertTriangle className="h-8 w-8 text-[#f0c36a]" />
          </div>
          ...
          <h1 className="text-2xl font-black text-white tracking-tight mb-4">
            Peninjauan Peran Aktif
          </h1>

          <p className="text-sm text-[#c8d2bd] leading-relaxed mb-6">
            Halo,{" "}
            <strong className="text-white">
              {user?.name || "Fungsionaris"}
            </strong>
            . Akun Anda saat ini terdaftar dengan peran{" "}
            <strong className="text-[#a7f3d0]">Guest</strong>. Anda belum
            memiliki akses ke ruang kendali dashboard utama BEM FT.
          </p>

          <div className="rounded-2xl border border-white/5 bg-white/4 p-5 text-left text-xs text-[#a9b49c] leading-relaxed space-y-2 mb-8">
            <p className="font-semibold text-white mb-1">
              🔍 Apa yang harus saya lakukan?
            </p>
            <p>
              1. Silakan hubungi <strong>Kepala Departemen (Kadep)</strong> Anda
              atau <strong>Sekretaris Umum</strong> BEM FT UNESA.
            </p>
            <p>
              2. Minta fungsionaris administrator untuk meningkatkan peran akun
              Anda (misalnya ke peran <strong>Staff</strong> atau{" "}
              <strong>Kadep</strong>) di menu Pengguna IMS.
            </p>
            <p>
              3. Setelah peran Anda diubah di database, lakukan{" "}
              <strong>Keluar Sesi</strong> dan masuk kembali untuk menyinkronkan
              hak akses baru Anda.
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Button
              onClick={() => logout()}
              className="h-11 rounded-xl bg-white text-[#091c11] font-bold hover:bg-[#a7f3d0] hover:shadow-[0_0_24px_rgba(167,243,208,0.35)] transition-all cursor-pointer px-6"
            >
              Keluar Sesi
            </Button>
            <Button
              ...
              className="h-11 rounded-xl border border-white/10 bg-white/5 text-white hover:bg-white/8 transition-all cursor-pointer px-6"
            >
              Refresh Halaman
            </Button>
          </div>
  ...
  const totalBudget = stats?.usedBudget ?? activeBudget.used;
  ...
  const metrics = [
    {
      ...
      description: `${stats?.upcomingProker ?? 0} dalam planning`,
      icon: KanbanSquare,
    ...
    },
    {
      ...
      description: stats?.departmentRank
        ? `Rank ${stats.departmentRank} dari ${stats.totalDepartmentMembers || 0}`
        : "Belum masuk peringkat",
      icon: Award,
      trend: stats?.monthlyPointsChange
        ? `${stats.monthlyPointsChange >= 0 ? "+" : ""}${stats.monthlyPointsChange}`
        : "0",
      color: "#f0c36a",
    },
    {
      ...
      icon: Wallet,
      trend: stats?.monthlyBudgetChange
  ...
  if (activeRole === "KaBEM" || activeRole === "WaKaBEM") {
    return <KabemDashboard />;
  }

  if (activeRole === "Bendahara" || activeRole === "Bendahara BEM") {
    ...
    <div className="space-y-6 animate-in fade-in zoom-in-95 duration-500 ease-out">
        ...
        <div className="relative grid gap-6 lg:grid-cols-[1fr_380px]">
          <div className="min-w-0">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-[#c8d2bd]">
              Ringkasan operasional kabinet, keuangan, proposal, dan kepanitiaan
              dalam satu ruang kendali.
            </p>
            <div className="mt-6 flex flex-wrap gap-3">
              <Button className="group relative overflow-hidden h-10 rounded-full bg-white px-5 text-[#091c11] font-semibold hover:bg-[#a7f3d0] hover:shadow-[0_0_24px_rgba(167, 243, 208,0.45)] hover:-translate-y-0.5 transition-all duration-300 active:translate-y-0 cursor-pointer">
                ...
                <span className="relative z-10">Review Proposal</span>
              </Button>
              <Button
                variant="ghost"
              ...
              >
                {/* Reflective shimmer effect for outline button */}
      ...
      {(isStatsLoading || isActivitiesLoading || isAgendaLoading) && (
        <div className="inline-flex items-center gap-2 rounded-full border border-[#10b981]/20 bg-[#10b981]/10 px-3 py-1.5 text-xs text-[#a7f3d0]">
          ...
          Menyinkronkan data operasional
        </div>
      )}
...
function ActivityFeed({
  ...
  if (items.length === 0) {
    return (
      <div className="mt-5 flex flex-col items-center justify-center rounded-lg border border-dashed border-white/10 bg-white/5 py-8 text-center">
        <ActivityIcon className="h-8 w-8 text-[#a9b49c]/50" />
