    ...
    PublicApiService.getNews({ page: 1, limit: 4 })
      .then((res) => {
        if (res?.data) {
          ...
          setRecommendations(res.data.filter((item) => item.slug !== slug).slice(0, 3));
        }
        setRecLoading(false);
      })
      .catch((err) => {
  ...
  if (loading) {
    return (
      <div className="pt-32 pb-24 px-6 max-w-6xl mx-auto w-full animate-pulse">
        <div className="h-4 bg-slate-green/40 rounded w-1/4 mb-8" />
        <div className="h-10 bg-slate-green/50 rounded w-3/4 mb-4" />
        <div className="h-4 bg-slate-green/40 rounded w-1/3 mb-12" />
        <div className="space-y-4">
          <div className="h-4 bg-slate-green/40 rounded w-full" />
          <div className="h-4 bg-slate-green/40 rounded w-full" />
          <div className="h-4 bg-slate-green/40 rounded w-5/6" />
        </div>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 mt-6">
        ...
        <section className="lg:col-span-2">
          ...
          <div className="mb-8">
            ...
            {/* Metadata bar */}
            <div className="flex flex-wrap items-center gap-6 text-xs text-foreground/70 border-y border-sage/10 py-3.5">
          ...
          {/* Article Body */}
          <div className="glass-subtle border border-sage/15 hover:border-accent-gold/35 rounded-3xl p-6 md:p-10 relative overflow-hidden group">
            {/* Top scanline accent */}
            ...
            <div 
              className="text-sm text-foreground/85 leading-loose space-y-6 whitespace-pre-wrap prose prose-sm dark:prose-invert max-w-none prose-p:mb-4 prose-a:text-accent-gold hover:prose-a:text-sage prose-headings:text-foreground prose-strong:text-foreground"
        ...
        <aside className="lg:col-span-1 flex flex-col gap-8">
          <div className="glass-active border border-sage/15 rounded-3xl p-6 relative overflow-hidden">
            {/* Top scanline accent */}
            <div className="absolute top-0 left-0 w-full h-[2px] bg-accent-gold/20" />
            
            <h3 className="text-xs font-semibold text-accent-gold uppercase tracking-wide border-b border-sage/15 pb-3 mb-6">
              Rekomendasi Berita
            </h3>
            ...
            {recLoading ? (
              <div className="space-y-4 animate-pulse">
                {[1, 2].map((i) => (
                  <div key={i} className="h-16 bg-slate-green/50 rounded-xl w-full" />
                ))}
              </div>
              ...
              <div className="flex flex-col gap-6">
                {recommendations.map((rec) => (
                  <Link
                    ...
                    className="flex flex-col gap-2 group border-b border-sage/10 pb-4 last:border-0 last:pb-0"
                  >
                    <div className="flex items-center gap-3 text-xs text-foreground/60">
                      <span className="px-2 py-0.5 rounded bg-forest/50 text-sage border border-sage/10">
                        {rec.category}
                      </span>
