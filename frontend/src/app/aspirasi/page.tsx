...
export default function AspirasiPage() {
  ...
  const getStatusColor = (status: string) => {
    switch (status?.toLowerCase()) {
      case "done":
      case "resolved":
        return "text-sage border-sage/35 bg-sage/10";
      case "reviewed":
      case "process":
        return "text-accent-gold border-accent-gold/30 bg-accent-gold/10";
      default:
        return "text-foreground/50 border-sage/10 bg-slate-green/40 dark:bg-slate-green/5";
    }
  };
  ...
  return (
    <main className="pt-28 pb-16 px-6 max-w-6xl mx-auto w-full relative">
      ...
      <div className="mb-12 mt-6">
        <span className="text-xs font-semibold text-sage uppercase tracking-wide block mb-3">Kanal Advokasi</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 mt-6 items-start">
        ...
        <section className="lg:col-span-7 glass-subtle border border-sage/15 hover:border-accent-gold/30 rounded-3xl p-6 md:p-8">
          <h2 className="text-sm font-bold text-foreground uppercase tracking-widest border-b border-sage/10 pb-4 mb-6">
            Formulir Pengaduan
          </h2>

          {submitSuccess ? (
            <div className="p-8 text-center bg-forest/20 border border-sage/40 rounded-2xl flex flex-col items-center gap-4 animate-in zoom-in-95 duration-300">
              ...
              <div className="bg-slate-green/80 dark:bg-slate-green/10 border border-sage/30 rounded-xl px-4 py-3 font-mono text-xs text-accent-gold select-all font-bold tracking-wider my-2">
                {submitSuccess}
              </div>
              <button
        ...
        <section className="lg:col-span-5 flex flex-col gap-8">
          {/* Tracker box */}
          <div className="glass-active border border-sage/15 rounded-3xl p-6 md:p-8">
            <h2 className="text-xs font-semibold text-accent-gold uppercase tracking-wide border-b border-sage/15 pb-3 mb-6">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-grow px-4.5 py-3 rounded-xl bg-slate-green/60 border border-sage/15 text-xs text-foreground placeholder-foreground/50 focus:outline-none focus:border-accent-gold transition-colors"
                  />
                  <button
                    type="submit"
            ...
            <AnimatePresence>
              {trackedData && (
                <motion.div
                  ...
                  className="mt-6 border-t border-sage/10 pt-6 flex flex-col gap-5"
                >
                  ...
                  <div className="flex flex-col gap-3 p-4 rounded-xl bg-slate-green/60 border border-sage/10">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="p-4.5 rounded-xl bg-forest/20 border border-sage/20 text-xs text-foreground/80 leading-relaxed min-h-[60px]">
