...
export default function OprecDetailPage() {
  ...
  return (
    <main className="pt-28 pb-16 px-6 max-w-6xl mx-auto w-full relative min-h-screen">
      ...
      <Link 
        href="/oprec"
        className="inline-flex items-center text-xs font-bold text-sage uppercase tracking-wider hover:text-accent-gold transition-colors mb-8 mt-6"
      >
      ...
      <div className="flex flex-col lg:flex-row gap-12 items-start">
        ...
        <div className="w-full lg:w-1/3 shrink-0 flex flex-col gap-6 sticky top-28">
          <div className="relative aspect-[3/4] rounded-3xl overflow-hidden bg-slate-green/20 border border-sage/15 shadow-xl shadow-black/20">
            {recruitment.poster ? (
              <Image 
              ...
              <div className="w-full h-full flex flex-col items-center justify-center text-sage/30 p-6 text-center">
          ...
          <div className="glass-subtle p-6 rounded-2xl border border-sage/15 flex flex-col gap-4">
            <div className="flex items-center gap-3">
              <Calendar className="w-5 h-5 text-accent-gold shrink-0" />
              <div>
                <span className="block text-[10px] uppercase tracking-wider font-bold text-sage">Masa Pendaftaran</span>
            ...
            {recruitment.contactPerson && (
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="block text-[10px] uppercase tracking-wider font-bold text-sage">Narahubung</span>
                  <span className="text-sm font-medium text-foreground block">
            ...
            <div className="mt-4 pt-4 border-t border-sage/15">
              {isOpen && recruitment.formUrl ? (
                ...
                <button 
                  disabled
                  className="w-full bg-sage/10 text-sage/60 px-6 py-3.5 rounded-xl text-sm font-bold cursor-not-allowed text-center border border-sage/20"
                >
        ...
        <div className="flex-1 flex flex-col gap-10">
          ...
          {recruitment.positions && recruitment.positions.length > 0 && (
            <section>
              <h2 className="flex items-center gap-2 text-xl font-bold text-foreground mb-6 pb-2 border-b border-sage/15">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {recruitment.positions.map((pos: any, idx: number) => (
                  <div key={idx} className="glass-active border border-sage/20 p-5 rounded-2xl hover:border-accent-gold/30 transition-all">
                    ...
                    {pos.requirements && (
                      <div className="mt-3 pt-3 border-t border-sage/10">
                        <span className="text-[10px] uppercase font-bold text-sage tracking-wider block mb-1">Persyaratan Khusus</span>
          ...
          {recruitment.timeline && recruitment.timeline.length > 0 && (
            <section>
              ...
              <div className="relative pl-6 space-y-8 before:absolute before:inset-0 before:ml-[11px] before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-sage/20 before:to-transparent">
                    ...
                    <div key={idx} className="relative flex items-start gap-6">
                      <div className={`absolute -left-6 md:left-1/2 w-6 h-6 rounded-full flex items-center justify-center transform md:-translate-x-1/2 z-10 
                        ${isCurrent ? 'bg-accent-gold text-slate-900 ring-4 ring-accent-gold/20' : 
                          isPast ? 'bg-sage text-slate-900' : 'bg-slate-green border border-sage/30 text-sage'}`}
                      >
                      ...
                      <div className="md:w-1/2 md:pr-12 md:text-right md:ml-auto md:order-1 hidden md:block">
                        {item.startDate && (
                          <div className={`text-xs font-bold uppercase tracking-wider mb-1 ${isCurrent ? 'text-accent-gold' : 'text-sage'}`}>
                      ...
                      <div className="flex-1 md:w-1/2 md:pl-12 pt-0.5">
                        <div className={`text-xs font-bold uppercase tracking-wider mb-1 md:hidden ${isCurrent ? 'text-accent-gold' : 'text-sage'}`}>
