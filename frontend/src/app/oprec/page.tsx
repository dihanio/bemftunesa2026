...
export default function OprecPage() {
  ...
  useEffect(() => {
    const fetchRecruitments = async () => {
      try {
        ...
        const sorted = (res.data.data.data || res.data.data).sort((a: any, b: any) => {
  ...
  const getStatusBadge = (status: string, closeDate: string) => {
    ...
    return (
      <span className="bg-sage/10 text-sage border border-sage/20 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider">
        Ditutup
      </span>
    );
  };
  ...
  return (
    <main className="pt-28 pb-16 px-6 max-w-6xl mx-auto w-full relative min-h-screen">
      ...
      <div className="mb-12 mt-6">
        <span className="text-xs font-semibold text-sage uppercase tracking-wide block mb-3">Sistem Rekrutmen Terpadu</span>
      ...
      {loading ? (
        <div className="space-y-6">
          {[1, 2].map((i) => (
            <div key={i} className="animate-pulse w-full h-48 bg-sage/10 rounded-3xl" />
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 gap-6">
          {recruitments.map((rec: any) => {
            ...
            return (
              <div 
                key={rec._id}
                className="glass-active border border-sage/20 hover:border-accent-gold/40 rounded-3xl p-6 md:p-8 transition-all duration-300 flex flex-col md:flex-row gap-8 group"
              >
                {/* Poster Thumbnail */}
                <div className="w-full md:w-64 shrink-0">
                  <div className="aspect-[3/4] rounded-2xl overflow-hidden bg-slate-green/20 relative border border-sage/10">
                    {rec.poster ? (
                      <Image 
                ...
                <div className="flex-1 flex flex-col">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                    <div className="flex items-center gap-3 text-sm text-foreground/70 bg-slate-green/10 p-3 rounded-xl border border-sage/10">
                      <Calendar className="w-4 h-4 text-sage" />
                      <div>
                        <span className="block text-[10px] uppercase tracking-wider font-bold text-sage mb-0.5">Masa Pendaftaran</span>
                    ...
                    <div className="flex items-center gap-3 text-sm text-foreground/70 bg-slate-green/10 p-3 rounded-xl border border-sage/10">
                      ...
                      <div>
                        <span className="block text-[10px] uppercase tracking-wider font-bold text-sage mb-0.5">Posisi Dibutuhkan</span>
                        <span className="font-medium text-foreground">
                  ...
                  {/* Actions */}
                  <div className="mt-auto pt-4 border-t border-sage/10 flex flex-wrap items-center gap-4">
                    {isOpen && rec.formUrl && (
                      <a 
                        href={rec.formUrl}
                        target="_blank"
