  ...
  return (
    <main className="pt-28 pb-16 px-6 max-w-6xl mx-auto w-full relative">
      ...
      <div className="mb-12 mt-6">
        <span className="text-xs font-semibold text-accent-blue uppercase tracking-wide block mb-3">Profil Organisasi</span>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-12 mb-16 items-stretch">
        ...
        <div className="lg:col-span-1 flex flex-col gap-3">
          ...
          <button
            ...
            className={`w-full text-left p-5 rounded-2xl border transition-all duration-300 flex items-center gap-4 cursor-pointer ${
              activeTab === "visi"
                ? "border-accent-gold/40 bg-foreground/5 text-foreground font-bold shadow-sm"
        ...
        {/* Dynamic Details Box */}
        <div className="lg:col-span-2 glass-subtle border border-accent-blue/15 rounded-3xl p-8 md:p-10 flex flex-col justify-center min-h-[380px] overflow-hidden">
          {activeTab === "visi" && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300 flex flex-col gap-4">
              <span className="text-xs font-semibold text-accent-blue uppercase tracking-wide">Visi Organisasi</span>
          ...
          {activeTab === "misi" && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300 flex flex-col gap-4">
              <span className="text-xs font-semibold text-accent-blue uppercase tracking-wide">Misi Organisasi</span>
          ...
          {activeTab === "tujuan" && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300 flex flex-col gap-4">
              <span className="text-xs font-semibold text-accent-blue uppercase tracking-wide">Makna Nama</span>
          ...
          <div className="w-full lg:w-2/3 flex flex-col gap-4 relative z-10">
            {logoPhilosophy.map((philo: any, idx: number) => (
              <div
                ...
                className={`p-6 rounded-2xl bg-foreground/5 border-l-4 ${philo.color || 'border-accent-blue'} border-y border-r border-accent-blue/15 transition-all duration-300 cursor-default
                  ${hoveredPhilo === idx ? 'border-accent-gold/60 bg-foreground/10 scale-[1.02] shadow-[0_0_20px_rgba(234,179,8,0.15)]' : 'hover:border-accent-gold/30 hover:bg-foreground/10'}
                  ${hoveredPhilo !== null && hoveredPhilo !== idx ? 'opacity-60 scale-[0.98]' : 'opacity-100'}
                `}
              >
                <div className="flex items-center gap-4 mb-2">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300 ${hoveredPhilo === idx ? 'bg-accent-gold text-slate-900 shadow-md' : 'bg-foreground/10 text-foreground'}`}>
                    0{idx + 1}
                  </div>
      ...
      {/* Hymne Teknik Section */}
      <section className="max-w-2xl mx-auto text-center">
        <div className="glass-active border border-accent-blue/20 hover:border-accent-gold/30 rounded-3xl p-8 md:p-10 relative overflow-hidden group transition-all duration-300">
          <div className="absolute top-0 left-0 w-full h-[2px] bg-accent-gold/20" />
          
          <h2 className="text-xl font-extrabold text-foreground mb-6">Lirik Hymne Teknik</h2>
          <div className="text-xs text-foreground/75 leading-loose italic space-y-4">
            <p>
              Sebagai Bakti Padamu <br />
              Tanah Air Indonesia <br />
              Akal Budi Kan Kukembangkan Didalam Jiwa <br />
              Panji Persatuan Fakultas Teknik Nan Jaya
            </p>
            <p>
              Syukur Padamu Tuhan <br />
              Anugerah Telah Engkau Berikan <br />
              Sebagai Mahasiswa Teknik <br />
