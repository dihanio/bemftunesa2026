  ...
  const faqs = [
    {
      ...
      a: "Peminjaman inventaris dilakukan dengan mengunduh formulir peminjaman di portal IMS, mengisi rincian barang, meminta tanda tangan Ketua Ormawa peminjam, lalu menyerahkannya ke Biro Administrasi/Sekretariat BEM FT Gedung A1 Lantai 2 minimal 3 hari sebelum kegiatan."
    },
    {
      q: "Di mana lokasi Sekretariat BEM Fakultas Teknik UNESA?",
      a: "Sekretariat kami berlokasi di Gedung A1 Lantai 2, Kampus Ketintang Universitas Negeri Surabaya, Fakultas Teknik. Silakan kunjungi sekretariat kami pada jam operasional kerja."
    },
    {
  ...
  return (
    <main className="pt-28 pb-16 px-6 max-w-6xl mx-auto w-full relative">
      ...
      <div className="mb-12 mt-6">
        <span className="text-xs font-semibold text-sage uppercase tracking-wide block mb-3">Pusat Bantuan</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 mt-6 items-start">
        ...
        <section className="lg:col-span-7 flex flex-col gap-8">
          {/* Form */}
          <div className="glass-subtle border border-sage/15 hover:border-accent-gold/30 rounded-3xl p-6 md:p-8">
            <h2 className="text-sm font-bold text-foreground uppercase tracking-widest border-b border-sage/10 pb-4 mb-6">
              Formulir Pertanyaan
            </h2>
 
            {success ? (
              <div className="p-8 text-center bg-forest/50 border border-sage/40 rounded-2xl flex flex-col items-center gap-4 animate-in zoom-in-95 duration-300">
        ...
        <section className="lg:col-span-5 flex flex-col gap-6">
          <div className="glass-active border border-sage/15 rounded-3xl p-6 md:p-8 flex flex-col gap-5">
            <h2 className="text-xs font-semibold text-accent-gold uppercase tracking-wide border-b border-sage/15 pb-3">
              ...
              {faqs.map((faq, idx) => (
                <button
                  ...
                  className={`w-full text-left p-4 rounded-2xl border transition-all duration-300 flex items-center gap-3 cursor-pointer ${
                    activeFaq === idx
                      ? "border-accent-gold/40 bg-slate-green/80 dark:bg-slate-green/10 text-foreground font-semibold shadow-sm"
                      : "border-sage/15 bg-slate-green/10 text-foreground/70 hover:text-foreground hover:bg-slate-green/30"
                  }`}
                >
                  <span className="text-[10px] font-bold text-accent-gold bg-forest/40 border border-accent-gold/20 w-6 h-6 rounded-full flex items-center justify-center shrink-0">
                    {idx + 1}
                  </span>
            ...
            <div className="glass-subtle border border-sage/10 rounded-2xl p-5 flex flex-col justify-center h-[200px] overflow-hidden">
              <span className="text-[10px] font-semibold text-sage uppercase tracking-wider block mb-2">Jawaban Bantuan</span>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="p-5 rounded-2xl bg-slate-green/80 dark:bg-slate-green/5 border border-sage/10 hover:border-accent-gold/45 hover:-translate-y-0.5 hover:shadow-sm transition-all duration-300 flex flex-col gap-2 group">
