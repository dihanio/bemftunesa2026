...
export function Breadcrumbs({ items }: BreadcrumbsProps) {
  return (
    <div className="inline-flex flex-wrap items-center gap-2.5 py-3 px-5 rounded-2xl glass-subtle border border-white dark:border-accent-blue/15 max-w-full font-mono text-[10px] uppercase tracking-widest text-accent-blue shadow-sm mb-8 animate-in fade-in slide-in-from-left-4 duration-300">
      ...
      {items.map((item, idx) => (
        <React.Fragment key={idx}>
          <ChevronRight className="w-3 h-3 text-accent-blue/40 shrink-0" />
