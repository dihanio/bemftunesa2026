  ...
  return (
    <nav
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ${
        isScrolled
          ? "py-4 glass-overlay shadow-md border-b border-sage/15"
    ...
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-3.5 group shrink-0">
          <Image src="/images/logo-bemft.png" alt="Logo BEM FT" width={44} height={44} className="object-contain drop-shadow-md transition-transform duration-300 group-hover:scale-110 group-hover:rotate-3 shrink-0" />
              ...
              return (
                <div key={idx} className="relative group py-2">
                  ...
                  <div className="absolute top-full left-1/2 -translate-x-1/2 pt-4 opacity-0 translate-y-2 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto transition-all duration-300 z-50">
                    <div className="glass-active border border-sage/15 rounded-2xl shadow-xl shadow-black/10 py-3 w-56 flex flex-col relative overflow-hidden">
                      <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-accent-gold to-yellow-400" />
                      {link.children?.map((child) => (
                        <Link
                          key={child.path}
                          href={child.path}
                          className={`px-5 py-2.5 text-xs font-bold tracking-widest uppercase transition-colors hover:bg-slate-green/20 ${
        ...
        <div className="hidden xl:flex items-center gap-3 xl:gap-4 shrink-0">
          <button
            ...
            className="p-2.5 rounded-full border border-sage/20 bg-slate-green/45 text-sage hover:text-accent-gold hover:border-accent-gold/40 hover:bg-sage/10 transition-all duration-300 cursor-pointer"
          ...
          <Link
            href="/aspirasi"
            className="px-5 py-2.5 rounded-full border border-accent-gold/30 bg-transparent text-[10px] font-bold uppercase tracking-widest text-accent-gold hover:bg-foliage hover:text-white hover:border-foliage transition-all duration-300 flex items-center gap-2"
          >
        ...
        <button
          ...
          className="xl:hidden p-2 rounded-xl text-foreground/75 hover:text-foreground hover:bg-sage/10 transition-colors focus:outline-none"
      ...
      {isOpen && (
        <div className="xl:hidden absolute top-full left-0 w-full glass-overlay border-b border-sage/15 py-6 px-6 shadow-2xl flex flex-col gap-4 animate-in fade-in slide-in-from-top-5 duration-300 max-h-[80vh] overflow-y-auto">
          {navLinks.map((link, idx) => {
            if (link.isDropdown) {
              return (
                <div key={idx} className="flex flex-col gap-2">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-sage px-3 pt-2">
                    {link.name}
                  </span>
                  {link.children?.map((child) => {
                    ...
                    return (
                      <Link
                        ...
                        className={`text-xs font-bold uppercase tracking-widest py-2 px-3 rounded-xl transition-all duration-300 ml-2 ${
                          isActive
                            ? "bg-forest/50 text-accent-gold border-l-2 border-accent-gold pl-4"
                            : "text-foreground/75 hover:bg-sage/10 hover:text-foreground"
                        }`}
                      >
                        {child.name}
                      </Link>
          ...
          <div className="w-full h-px bg-sage/15 my-2" />
          <button
            onClick={() => {
              toggleSwitchTheme();
              setIsOpen(false);
            }}
            className="flex items-center justify-between text-xs font-bold uppercase tracking-widest py-2 px-3 rounded-xl text-foreground/75 hover:bg-sage/10 hover:text-foreground transition-all duration-300 w-full text-left cursor-pointer"
          >
          ...
          <Link
            href="/aspirasi"
            onClick={() => setIsOpen(false)}
            className="mt-2 w-full text-center px-5 py-3 rounded-full bg-foliage text-xs font-bold uppercase tracking-widest text-white hover:bg-sage transition-all duration-300"
          >
            Kirim Aspirasi
          </Link>
        </div>
