    ...
    <div className={`flex flex-col gap-2 ${className}`}>
      ...
      <div ref={wrapperRef} className="relative">
        <button
          ...
          className={`w-full px-4.5 py-3 rounded-xl bg-slate-green/60 border ${
            error ? "border-rose-500/60 focus:border-rose-500" : "border-sage/15 focus:border-sage/40"
        ...
        {isOpen && (
          <div className="absolute top-full left-0 right-0 mt-1.5 z-50 bg-forest border border-sage/15 rounded-xl shadow-2xl max-h-60 overflow-y-auto">
            {options.map((option) => (
              <button
                key={option.value}
                type="button"
