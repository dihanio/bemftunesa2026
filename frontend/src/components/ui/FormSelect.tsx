          ...
          <div className="absolute top-full left-0 right-0 mt-1.5 z-50 bg-slate-800 border border-accent-blue/15 rounded-xl shadow-2xl max-h-60 overflow-y-auto">
            {options.map((option) => (
              <button
                ...
                className={`w-full px-4 py-2.5 text-sm text-left flex items-center justify-between gap-2 hover:bg-accent-blue/10 cursor-pointer transition-colors ${
                  value === option.value ? "text-accent-blue" : "text-foreground/80"
                }`}
              >
                {option.label}
