...
/* ═══════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════ */
function Viewport1Monument({ progress }: { progress: MotionValue<number> }) {
  const opacity = useTransform(progress, [0, 0.05, 0.15, 0.22], [0, 1, 1, 0]);
  const scale = useTransform(progress, [0, 0.18], [1.15, 0.95]);
  const lineDraw = useTransform(progress, [0.02, 0.16], [0, 1]);
  const mascotLeftX = useTransform(progress, [0, 0.18], [-80, 0]);
  const mascotRightX = useTransform(progress, [0, 0.18], [80, 0]);
  ...
  return (
    ...
    >
      ...
      <motion.div style={{ scale }} className="space-y-6 relative z-10 max-w-4xl bg-[#040507]/60 p-6 rounded-2xl backdrop-blur-[2px]">
...
/* ═══════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════ */
function Viewport2Manifesto({ progress }: { progress: MotionValue<number> }) {
  const opacity = useTransform(progress, [0.20, 0.25, 0.42, 0.48], [0, 1, 1, 0]);
  const textX = useTransform(progress, [0.22, 0.35], [-60, 0]);
  const imageClip = useTransform(progress, [0.25, 0.40], ["inset(20% 20% 20% 20%)", "inset(0% 0% 0% 0%)"]);
  const imageScale = useTransform(progress, [0.25, 0.40], [1.2, 1]);
...
/* ═══════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════ */
function Viewport3Companions({ progress }: { progress: MotionValue<number> }) {
  const opacity = useTransform(progress, [0.45, 0.50, 0.67, 0.73], [0, 1, 1, 0]);
  const prishaX = useTransform(progress, [0.48, 0.62], [-100, 0]);
  const smayaX = useTransform(progress, [0.48, 0.62], [100, 0]);
  const textY = useTransform(progress, [0.48, 0.60], [10, 0]);
...
/* ═══════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════ */
function Viewport4Pillars({ progress }: { progress: MotionValue<number> }) {
  const opacity = useTransform(progress, [0.70, 0.74, 0.85, 0.90], [0, 1, 1, 0]);

  // Column Highlight Multipliers
  const col1Opacity = useTransform(progress, [0.72, 0.77], [0.3, 1]);
  const col2Opacity = useTransform(progress, [0.76, 0.81], [0.3, 1]);
  const col3Opacity = useTransform(progress, [0.80, 0.85], [0.3, 1]);
...
/* ═══════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════ */
function Viewport5Finale({ progress }: { progress: MotionValue<number> }) {
  const opacity = useTransform(progress, [0.88, 0.92], [0, 1]);
  const scale = useTransform(progress, [0.88, 0.96], [0.95, 1]);
