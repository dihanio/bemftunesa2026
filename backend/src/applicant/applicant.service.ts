...
export class ApplicantService {
  ...
  private async pushStatusHistory(
    ...
    const updatedApplicant = await this.applicantModel.findByIdAndUpdate(
      id,
      {
        status,
  ...
  async register(recruitmentId: string, dto: RegisterApplicantDto) {
    ...
    if (recruitment.status !== 'open')
      throw new BadRequestException('Pendaftaran rekrutmen ini sedang ditutup');
    if (!recruitment.useInternalForm)
      throw new BadRequestException('Rekrutmen ini menggunakan form eksternal');

    // Check duplicate NIM or Email
    const existingNim = await this.applicantModel
      .findOne({
    ...
    if (existingNim)
      throw new ConflictException(
        `NIM ${dto.nim} sudah terdaftar pada rekrutmen ini`,
      );

    const existingEmail = await this.applicantModel
      .findOne({
        recruitmentId: new Types.ObjectId(recruitmentId),
        email: dto.email,
      })
    ...
    if (existingEmail)
      throw new ConflictException(
        `Email ${dto.email} sudah terdaftar pada rekrutmen ini`,
      );
  ...
  async updateStatus(
    ...
    if (dto.notes) {
      await this.applicantModel.findByIdAndUpdate(id, {
  ...
  async scheduleInterview(
    ...
    if (dto.interviewerId)
      interviewData.interviewerId = new Types.ObjectId(dto.interviewerId);
    if (dto.interviewerName)
      interviewData.interviewerName = dto.interviewerName;
    ...
    await this.pushStatusHistory(
      ...
      'interview_scheduled',
      userId,
      `Interview scheduled at ${dto.scheduledAt} in ${dto.location}`,
    );
  ...
  async submitInterviewResult(
    ...
    if (!applicant.interview)
      throw new BadRequestException('Wawancara belum dijadwalkan');
    ...
    if (s.communication !== undefined) {
      finalScore += s.communication;
      count++;
    }
    if (s.motivation !== undefined) {
      finalScore += s.motivation;
      count++;
    }
    if (s.teamwork !== undefined) {
      finalScore += s.teamwork;
      count++;
    }
    if (s.leadership !== undefined) {
      finalScore += s.leadership;
      count++;
    }
    if (s.technical !== undefined) {
      finalScore += s.technical;
      count++;
    }
  ...
  async checkResult(recruitmentId: string, nimOrEmail: string) {
    const filter = {
      recruitmentId: new Types.ObjectId(recruitmentId),
      $or: [{ nim: nimOrEmail }, { email: nimOrEmail.toLowerCase() }],
    };
    const applicant = await this.applicantModel
      .findOne(filter)
      .select('name status positionChoice interview statusHistory')
      .exec();
    if (!applicant)
