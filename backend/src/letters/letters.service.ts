...
import { ROLES_BPI, ROLE_SEKRETARIS_ADMINISTRASI, ROLE_SEKRETARIS_KEGIATAN } from '../common/constants/roles';
...
export class LettersService {
  ...
  async findAll(query: QueryLetterDto, userRole: string, userDepartment?: string) {
  ...
  async create(dto: CreateLetterDto, userId: string, userDepartment?: string) {
    const letter = await this.letterModel.create({
      ...
      department: userDepartment || dto.department ? new Types.ObjectId(userDepartment || dto.department) : undefined,
  ...
  async update(id: string, dto: UpdateLetterDto, userRole: string, userDepartment?: string) {
    const letter = await this.findById(id);

    // RBAC checking
    const isBPI = (ROLES_BPI as readonly string[]).includes(userRole);
    if (!isBPI && userDepartment && letter.department?.toString() !== userDepartment) {
      throw new ForbiddenException('You can only edit letters from your own department');
    }

    // Logic for Auto-Increment Reference Number when approved
    if (dto.status === 'approved' && letter.status !== 'approved' && ['outgoing', 'proposal', 'lpj'].includes(letter.type)) {
      ...
      const lastLetter = await this.letterModel.findOne({
        type: letter.type,
        referenceNumber: { $exists: true },
        dateApproved: { $gte: startOfYear, $lte: endOfYear }
      }).sort({ dateApproved: -1 }).exec();
  ...
  async calculateSmartPriority() {
    ...
    const matrix = letters.map(letter => {
      ...
      if (letter.deadlineDate) {
        const daysLeft = Math.floor((new Date(letter.deadlineDate).getTime() - now) / (1000 * 60 * 60 * 24));
        // Inverse: semakin sedikit sisa hari, semakin besar nilainya
        if (daysLeft <= 0) c1 = 100;       // Sudah lewat deadline
      ...
      // C2: Skala Dampak
      const impactMap: Record<string, number> = { internal: 1, fakultas: 2, universitas: 3, eksternal: 4 };
      const c2 = impactMap[letter.impactScale] || 1;

      // C3: Tingkat Kepentingan
      const urgencyMap: Record<string, number> = { normal: 1, high: 2, urgent: 3 };
      const c3 = urgencyMap[letter.urgencyLevel] || 1;

      // C4: Lama mengendap pada status saat ini (hari sejak terakhir diupdate)
      const stagnancyTime = letter.updatedAt ? new Date(letter.updatedAt).getTime() : now;
    ...
    const finalScores = normalized.map(u => {
      const v = (u.u1 * weights.c1) + (u.u2 * weights.c2) + (u.u3 * weights.c3) + (u.u4 * weights.c4);
  ...
  async getStats() {
    ...
    const pending = await this.letterModel.countDocuments({ ...filter, status: 'pending' });
    const approved = await this.letterModel.countDocuments({ ...filter, status: 'approved' });
    const rejected = await this.letterModel.countDocuments({ ...filter, status: 'rejected' });
    
    return {
      total: { label: 'Total Surat', value: total },
      pending: { label: 'Menunggu Persetujuan', value: pending, color: 'warning' as const },
      approved: { label: 'Disetujui', value: approved, color: 'success' as const },
