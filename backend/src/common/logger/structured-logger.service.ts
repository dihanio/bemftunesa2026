...
export class StructuredLogger implements LoggerService {
  ...
  private formatMessage(
    ...
    if (optionalParams && optionalParams.length > 0) {
      if (typeof optionalParams[optionalParams.length - 1] === 'object') {
        meta = optionalParams[optionalParams.length - 1] as Record<
          string,
          unknown
        >;
      }
    ...
    if (isProd) {
      ...
      const metaStr = Object.keys(meta).length
        ...
        : '';
      return `${color}[${level.toUpperCase()}]${reset} [${timestamp}] [${activeContext}] ${String(message)}${metaStr}`;
    }
  }
  ...
  log(message: unknown, context?: string, ...optionalParams: unknown[]) {
    console.log(
      this.formatMessage('info', message, context, ...optionalParams),
    );
  }
